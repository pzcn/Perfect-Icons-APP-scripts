install() {
    echo "'@string:string_installing'$theme_name..."
    cd theme_files/miui
    ...
    [ $addon == 1 ] && cp -rf $addon_path/'@string:string_advancedaddons'/* $FAKEMODPATH/system/media/theme/default 2>/dev/null
...
addon(){
    ...
    cp -rf $addon_path/'@string:string_animatingicons'/* $TEMP_DIR/layer_animating_icons/ 2>/dev/null
    cp -rf $addon_path/'@string:string_staticicons'/* $TEMP_DIR/res/drawable-xxhdpi/ 2>/dev/null
    cd $TEMP_DIR
    zip -r icons.zip res >/dev/null
    zip -r icons.zip layer_animating_icons >/dev/null
    cd ..
...
require_new_magisk() {
  echo
  echo "@string:string_MagiskNotSupport"
  echo
  echo "@string:string_NeedNewMagisk"
  echo
  exit 1
}
...
source $START_DIR/online-scripts/misc/downloader.sh
  if [ $var_version -lt 10 ]; then 
    echo "@string:string_SysVerNotSupport"
    rm -rf $TEMP_DIR/*
    exit 1
  elif [ $var_miui_version -ge 10 ]; then
  echo "@string:string_startinstallation"
  fi
  [ "`curl -I -s --connect-timeout 1 https://miuiiconseng-generic.pkg.coding.net/iconseng/engtest/test?version=latest -w %{http_code} | tail -n1`" == "200" ] || {  echo "@string:string_nonetworkdetected"&& rm -rf $TEMP_DIR/* 2>/dev/null && exit 1; }
  echo ""
...
name=MIUI '@string:string_projectname'
author=@PedroZ
description='@string:string_moduledescription_1'${theme_name}'@string:string_moduledescription_2'
...
themeid=$var_theme" >> $TEMP_DIR/module.prop
  ...
  echo ""
  echo "@string:string_installsuccess"
  echo "---------------------------------------------"
  exit 0