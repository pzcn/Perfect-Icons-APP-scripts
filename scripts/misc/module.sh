...
transform_config() {
  if [ $new_transform_config = 1 ]; then
    cp -rf $START_DIR/online-scripts/misc/transform_config2.xml $TEMP_DIR/transform_config.xml
  else
    cp -rf $START_DIR/online-scripts/misc/transform_config1.xml $TEMP_DIR/transform_config.xml
  fi
}
