downloader() {
    ...
    sh $START_DIR/online-scripts/misc/am.sh start -a android.intent.action.MAIN -n "$activity" --es downloadUrl "$downloadUrl" --ez autoClose true --es taskId "$task_id" 1 > /dev/null
